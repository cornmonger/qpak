...
mod tests {
    ...
    fn test_fixture_pak() {
        ...
        do_test_pak(fixture_pak_file);
    }

    fn do_test_pak<P: AsRef<Path>>(pak_file: P) {
        let test_pak_path = PathBuf::from(pak_file.as_ref());
        let dest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("target/tests/qpak/test_installed_pak/test_pak");
        let new_test_pak_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("target/tests/qpak/test_installed_pak/new_test_pak.pak");
        let new_dest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("target/tests/qpak/test_installed_pak/new_test_pak");
